"""English financial sentiment analyzer using ProsusAI/finbert."""
...
class EnglishSentimentAnalyzer(BaseSentimentAnalyzer):
    """Financial sentiment for English texts using FinBERT (3-class).
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            logger.error(f"FinBERT load failed: {exc}")
            self._pipeline = None
    ...
    def analyze_batch(self, texts: Sequence[str]) -> tuple[SentimentResult, ...]:
        ...
        for i in range(0, len(text_list), self._batch_size):
            ...
            try:
                ...
                logger.warning(f"FinBERT batch error: {exc}")
